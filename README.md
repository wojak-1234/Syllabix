...
## 🖥️ 4. 웹 인터페이스 경로 명세 (Frontend Routes)

사용자가 브라우저를 통해 직접 접속하는 주요 페이지 경로입니다.

### 🏠 일반 및 공통
- `/`: **Syllabix 홈** (서비스 소개 및 랜딩 페이지)
- `/onboarding`: **온보딩** (사용자 이름, 학습 목표, 관심 분야 설정)
- `/start`: **설계 시작** (AI 커리큘럼 생성을 위한 페르소나 입력)
- `/chatbot`: **Syllabix 챗봇** (프로젝트 가이드 및 서비스 상담)

### 📖 Student Experience (학습자 경로)
- `/curriculum`: **커리큘럼 프리뷰** (AI가 설계한 로드맵 확인 및 수강 결정)
- `/dashboard`: **학습 대시보드** (수강 중인 강좌 목록 및 전체 진도 관리)
- `/learn/[seriesId]`: **강좌 메인** (강좌 전체 목차 및 상세 개요)
- `/learn/[seriesId]/lecture/[lectureId]`: **강의실** (본문 학습, 퀴즈 및 AI 튜터링)
- `/learn/[seriesId]/lecture/[lectureId]/coding-test`: **코딩 실습실** (강의 주제 관련 알고리즘 테스트)

### 📊 Teacher Experience (교수자 경로)
- `/teacher/dashboard`: **교수자 센터** (보유 강좌 관리 및 새 강좌 기획)
- `/teacher/series/[id]/edit`: **강좌 편집기** (커리큘럼 수정, 강의 내용 AI 생성 및 배포)
- `/teacher/analytics`: **학습 분석** (수강생 전체 데이터 기반 인사이트 리포트)

---

## 📑 5. 시스템 문서 체계 (MD Documents)
...
## 💡 6. 개발 및 IDE 접근법 (Developer Guide)
