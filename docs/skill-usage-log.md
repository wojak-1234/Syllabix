...
#### Skill: Phase 2-C — AI 문제 출제 (퀴즈/코딩테스트) 프롬프트 및 API 구현
- Timestamp: 2026-04-12T16:48:00+09:00
- Task Description:
Teacher UI에 연결될 AI 문제 생성 기능(퀴즈, 코딩테스트)의 프롬프트와 백엔드 API를 구현.

- Reason for Using This Skill:
Teacher가 일일이 문제를 출제하지 않아도 '강좌의 학습 목표와 개념 태그' 기반으로 AI가 자동 출제(T-02 명세)할 수 있도록 하기 위함.

- Execution Summary:
1. `lib/prompts.ts`: `buildGenerateQuizPrompt`, `buildGenerateCodingTestPrompt` 추가. 응답을 JSON 형태로 제한하고 COMMON_RULES 적용.
2. `api/teacher/quiz/generate/route.ts`: 퀴즈 출제 API (MODELS.STANDARD 모델 사용).
3. `api/teacher/coding-test/generate/route.ts`: 코딩테스트 출제 API (MODELS.PRO 모델 사용 - 높은 추론 요구).

- Skills Applied:
  - `coding-guidelines`: 프롬프트 시스템을 `lib/prompts.ts` 한 곳에 중앙화하여 결합도를 낮춤.
- Timestamp: 2026-04-12T05:01:00+09:00
- Task Description:
수요 키워드 클릭 → 원클릭으로 강의 목차·설명·문제 세트를 AI가 3단계 순차 자동 생성.
...
- Skills Applied:
  - `coding-guidelines`: 프롬프트 시스템을 `lib/prompts.ts` 한 곳에 중앙화하여 결합도를 낮춤.
  - `backend-security-coder`: API 단에서 누락 필드 검증 수행 및 예외 처리 구색.

- Result:
AI를 이용한 문제 자동 생성 백엔드 시스템 구축 완료. UI에서 연결만 하면 기능이 동작하도록 세팅됨.

- User Intervention:
No.

#### Skill: Phase 3-A — 학생 대시보드 리뉴얼 및 수강 API
- Timestamp: 2026-04-12T16:55:00+09:00
- Task Description:
기존의 정적인 커리큘럼 생성 화면이었던 대시보드를 수강 중인 Series와 추천 Series를 보여주는 화면으로 개편, Enrollment API 구축.

- Reason for Using This Skill:
project-overview v3 설계상, 학생은 챗봇(AI)에게 커리큘럼 자체를 뽑아내는 것이 아니라, Teacher가 만든 Series 중에서 AI가 연결(추천)해주는 형태임. 따라서 대시보드는 이를 열람하는 형태로 완전히 바뀌어야 함.

- Execution Summary:
1. `api/student/enrollments/route.ts`: 복합 Unique Key(`studentId_seriesId`)를 확인하여 중복 등록 방지.
2. `app/dashboard/page.tsx`: UI 코드 전면 개편. 수강 중인 내역과, AI 추천 카드 형태로 변환.

- Skills Applied:
  - `coding-guidelines`: 하나의 책임(Student View 전환)에 집중.
  - `ui-builder`: 직관적인 레이아웃 및 게이미피케이션 요소(진도바, 뱃지) 활용.

- Result:
학생이 로그인 후 가장 먼저 마주하는 View가 v3의 취지(Teacher 개입 기반)를 완벽히 반영하도록 수정됨.
